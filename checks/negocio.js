 ...
 * WhatsApp, tel clickeable, direccion fisica, favicon,
 * cookies/GDPR, politica de privacidad, redes sociales, SSL vencimiento
 */
...
async function checkNegocio({ url, page }) {
  ...
  checks.push({
    ...
    detalle: whatsappInfo.numeros.length > 0 || whatsappInfo.widgetDetectado
      ...
      : 'No se detectaron botones ni links de WhatsApp',
  ...
  const telefonosInfo = await page.evaluate(() => {
    ...
    const numerosEnTexto = (textoCompleto.match(/(?:\+?52\s?)?(?:\(?\d{2,3}\)?\s?)?\d{4}[-\s]?\d{4}/g) || []).length;
  ...
  if (telefonosInfo.clickeables.length > 0) {
    checks.push({
      ...
      estado: 'OK',
      detalle: `${telefonosInfo.clickeables.length} numero(s) clickeable(s) en mobile`,
      items: telefonosInfo.clickeables.map(t => `${t.numero} — "${t.texto}"`)
    });
  } else if (telefonosInfo.numerosEnTexto > 0) {
    checks.push({ nombre: 'Telefono clickeable', estado: 'ADVERTENCIA', detalle: 'Hay numeros de telefono en el texto pero NO son clickeables en mobile' });
  } else {
    checks.push({ nombre: 'Telefono clickeable', estado: 'ADVERTENCIA', detalle: 'No se detecto numero de telefono en la pagina' });
  }

  // 3. Direccion fisica
  const tieneDireccion = await page.evaluate(() => {
    const texto = document.body.innerText.toLowerCase();
    const indicadores = ['calle ', 'av.', 'avenida', 'blvd', 'boulevard', 'col.', 'colonia', 'c.p.', 'codigo postal', 'address'];
    return !!document.querySelector('address') || indicadores.some(i => texto.includes(i));
  });

  checks.push({
    nombre: 'Direccion fisica',
    estado: tieneDireccion ? 'OK' : 'ADVERTENCIA',
    detalle: tieneDireccion
      ? 'Se detecta informacion de direccion fisica — positivo para SEO local'
      : 'No se detecto direccion fisica — importante para SEO local'
  });
  ...
  checks.push({
    nombre: 'Favicon',
    estado: favicon ? 'OK' : 'ADVERTENCIA',
    detalle: favicon ? 'Favicon configurado' : 'Sin favicon — muestra el icono generico del browser'
  });

  // 5. Aviso de cookies / GDPR
  ...
  checks.push({
    ...
    detalle: tieneCookies
      ...
      : 'No se detecto aviso de cookies — requerido si usas tracking'
  });
  ...
  checks.push({
    ...
    estado: privacidad.encontrado ? 'OK' : 'ERROR',
    detalle: privacidad.encontrado
      ? `Enlace encontrado: "${privacidad.texto}"`
      : 'Sin politica de privacidad — obligatorio si tienes formularios o tracking'
  });

  // 7. Links a redes sociales
  const redes = await page.evaluate(() => {
    ...
    return Object.entries(dominios)
      .map(([red, dominio]) => {
        ...
        return link ? { red, url: link.href } : null;
      })
      .filter(Boolean);
  });
  ...
  checks.push({
    ...
    detalle: redes.length > 0
      ? `${redes.length} red(es) enlazada(s): ${redes.map(r => r.red).join(', ')}`
    ...
    try {
      ...
      const socket = tls.connect(443, hostname, { servername: hostname }, () => {
        ...
        if (!cert || !cert.valid_to) {
          return resolve({ nombre: 'Certificado SSL', estado: 'ADVERTENCIA', detalle: 'No se pudo obtener info del certificado' });
        }
        const dias = Math.floor((new Date(cert.valid_to) - new Date()) / (1000 * 60 * 60 * 24));
        if (dias < 0) resolve({ nombre: 'Certificado SSL', estado: 'ERROR', detalle: 'El certificado SSL ha EXPIRADO — el sitio muestra "No seguro"' });
      ...
      resolve({ nombre: 'Certificado SSL', estado: 'ADVERTENCIA', detalle: 'No se pudo verificar el certificado' });
    }
  });
}
