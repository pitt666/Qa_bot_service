 ...
 * Deteccion, envio real con Mailtrap, pagina de gracias, anti-spam
 */
...
async function checkFormularios({ page, context, mailtrap_token, mailtrap_inbox_id }) {
  ...
  if (formularios.length === 0) {
    ...
    checks.push({
      ...
      items: formularios.map(f => `Formulario ${f.indice}: ${f.totalCampos} campo(s) — Submit: ${f.tieneSubmit ? f.textoSubmit : 'SIN BOTON DE ENVIO'}`)
    });

    // Prueba de envio real con Mailtrap
    if (mailtrap_token && mailtrap_inbox_id) {
      ...
      checks.push({
        ...
        estado: 'ADVERTENCIA',
        detalle: 'Agrega mailtrap_token y mailtrap_inbox_id al request para probar el envio real del formulario'
      });
    }
  }
  ...
  checks.push({
    ...
    detalle: tienePaginaGracias
      ? 'Pagina de gracias detectada — el Pixel y GA4 pueden trackear conversiones correctamente'
      : 'No se detecto pagina de gracias — sin esto el Pixel y GA4 no pueden trackear conversiones'
  });
  ...
  checks.push({
    ...
    detalle: tieneRecaptcha
      ...
      : formularios.length > 0
        ? 'Sin reCAPTCHA — los formularios pueden recibir spam'
...
async function intentarEnvioFormulario(page, formulario, mailtrap_token, mailtrap_inbox_id) {
  try {
    ...
    if (!submitBtn) return { nombre: 'Prueba de envio real', estado: 'ADVERTENCIA', detalle: 'Sin boton de envio para hacer la prueba' };
    ...
    try {
      const r = await fetch(`https://mailtrap.io/api/accounts/1/inboxes/${mailtrap_inbox_id}/messages`, {
    ...
    return { nombre: 'Prueba de envio real', estado: 'ADVERTENCIA', detalle: `No se pudo completar la prueba: ${e.message}` };
  }
}
